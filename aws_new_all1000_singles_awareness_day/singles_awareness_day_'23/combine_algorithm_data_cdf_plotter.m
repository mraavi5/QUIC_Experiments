...
%colors = ["#45788C", "#40372F", "#BF5841"]

line_width = 3;
font_size = 22;
legend_font_size = 20;
legend_pos = 'NorthOutside'

f = figure;
f.Position = [100 100 1150 550];
hold on;
...
y1q = data_quic(:, 1)
c1q = cdfplot(y1q);
set(c1q,'LineWidth', line_width);
set(c1q, 'LineStyle', '-', 'Color', colors{1});
if drawAverages == 1
	avg = mean(y1q, 1, 'omitnan');
	xline(avg, 'LineWidth', line_width, 'LineStyle', ':', 'Color', colors{1}, 'HandleVisibility', 'off');
end
...
y4q = data_quic(:, 4)
c4q = cdfplot(y4q);
set(c4q,'LineWidth', line_width);
set(c4q, 'LineStyle', '-', 'Color', colors{4});
if drawAverages == 1
	avg = mean(y4q, 1, 'omitnan');
	xline(avg, 'LineWidth', line_width, 'LineStyle', ':', 'Color', colors{4}, 'HandleVisibility', 'off');
end
...
y6q = data_quic(:, 6)
c6q = cdfplot(y6q);
set(c6q,'LineWidth', line_width);
set(c6q, 'LineStyle', '-', 'Color', colors{6});
if drawAverages == 1
	avg = mean(y6q, 1, 'omitnan');
	xline(avg, 'LineWidth', line_width, 'LineStyle', ':', 'Color', colors{6}, 'HandleVisibility', 'off');
end
...
l = legend([c1, c4, c6, c1q, c4q, c6q], 'TCP/TLS RSA 3072','TCP/TLS Dilithium 5','TCP/TLS Falcon 1024','QUIC RSA 3072','QUIC Dilithium 5','QUIC Falcon 1024', 'Location', legend_pos, 'numColumns', 3, 'Orientation', 'horizontal')
...
box on;
%axis square;
xlabel('Handshake Duration (ms)','FontSize',28);
ylabel('CDF','FontSize',28);
