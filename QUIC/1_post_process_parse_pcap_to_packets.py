...
def log(packet, fileName, file):
	...
	# Example file name: pending_TCPDUMP_dilithium2_numSamples_1000_0RTT_loss_15%_krPKqIdXJd
	file_algorithm = ''
	file_numSamples = ''
	file_rtt = ''
	file_experimentType = ''
	file_experimentVal = ''
	match = re.match(r'(?:pending_)?TCPDUMP_([^_]+)_numSamples_([^_]+)_([^_]+)_([^_]+)_([^_]+)', fileName)
	if match is not None:
		file_algorithm = match.group(1)
		file_numSamples = match.group(2)
		file_rtt = match.group(3)
		file_experimentType = match.group(4)
		file_experimentVal = match.group(5)
	else:
	...
	for layer in packet:
		...
		if layer.layer_name == 'quic':
			...
			if longPacketTypes[int(layer.get_field('long_packet_type') or 4)] == '':
				long_short += 'short '
				is_handshake_packet = False
			else:
				long_short += 'long '

	line = f'{timestamp},{timestamp_seconds},{file_algorithm},{file_numSamples},{file_experimentType},{file_experimentVal},{packetNum},{is_handshake_packet},{connectionCount},{successfulConnectionNum.strip()},{totalBytes},{quicBytes.strip()},{layers.strip()},{longPacketType.strip()},{frame.strip()},{frameType.strip()},{long_short.strip()}'
...
for pcapName in pcapFiles:
	...
	while running:
		try:
			...
			#first_quic_layer_index = -1
			#for i, layer in enumerate(packet.layers):
			#	if layer._layer_name == 'quic':
			#		first_quic_layer_index = i
			#		break
			...
			if packet.highest_layer == 'TCP':
				...
				log(packet, os.path.basename(pcapName), file)
