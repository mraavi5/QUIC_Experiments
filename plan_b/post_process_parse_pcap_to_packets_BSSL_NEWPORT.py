...
def log(packet, file, is_handshake_packet):
	...
	line = f'{timestamp},{timestamp_seconds},{packetNum},{is_handshake_packet},{connectionCount},{successfulConnectionNum.strip()},{totalBytes},{tlsBytes.strip()},{layers.strip()},{longPacketType.strip()},{frame.strip()},{frameType.strip()},{long_short.strip()}'
		...
		try:
			...
			if newConnectionCountReady and packet.layers[first_tcp_layer_index].flags == '0x00000014': # ACK RST
				...
				if packet.layers[first_tcp_layer_index].srcport == '4434' or packet.layers[first_tcp_layer_index].dstport == '4434':
					...
					log(packet, file, is_handshake_packet)
