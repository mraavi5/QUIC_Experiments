...
def network_delimeter(server_ip):
	terminal(f'nc -vz {server_ip} 4433 >/dev/null 2>/dev/null')
...
def startTcpdump(interface, algorithm, zeroRoundTrip):
	stopTcpdump()
	myCmd = f'python3 experiment_basic_run_tcpdump.py {interface} {algorithm} {zeroRoundTrip} {numSamples}'
	print(myCmd)
...
def restart_server_SSH(algorithm, server_ip):
	...
	output = call_ssh(f'python3 ~/oqs/lsquic/certs/plan_b/experiment_basic_server.py {algorithm} </dev/null &>/dev/null &')
	print()
...
if zeroRoundTrip:
	myCmd= f'{lsquic_dir}/build/bin/./http_client -0 0rttTest.txt -H www.example.com -s {server_ip}:4433 -p /'
else:
	myCmd= f'{lsquic_dir}/build/bin/./http_client -H www.example.com -s {server_ip}:4433 -p /'
