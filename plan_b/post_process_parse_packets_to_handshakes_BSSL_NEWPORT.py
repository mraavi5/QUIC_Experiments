...
for inputFileName in csvFiles:
	...
	for packet in reader:
		if packet[5] == '': continue
		#prev_sample_num = sample_num

		is_handshake_packet = packet[3].strip().upper() == 'TRUE'
		sample_num = int(packet[4])

		num_successful_connections = int(packet[5])
		...
		if is_handshake_packet:
			...
			handshake_bytes[num_successful_connections] += int(packet[6])

		else:
			payload_length[num_successful_connections] += 1
			payload_bytes[num_successful_connections] += int(packet[6])

		# Count number of long and short headers
		long_short_list = packet[12].split(' ')
		quic_bytes = packet[7].split(' ')
	...
	# 	packet_num = packet[2]
	# 	is_handshake_packet = packet[3]
	# 	num_connections = packet[4]
	# 	num_successful_connections = packet[5]
	# 	total_bytes = packet[6]
	# 	quic_bytes = packet[7]
	# 	layers = packet[8]
	# 	long_packet_type = packet[9]
	# 	frame = packet[10]
	# 	frame_type = packet[11]
	# 	long_short_headers = packet[12]
